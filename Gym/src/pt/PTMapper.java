...
public class PTMapper implements ResultMapper<PT> {
	...
	public PT resultMapping(ResultSet rs) {
		try {
			...
			return PT.builder().PT_Trainer_ID(PT_Trainer_ID).PT_Member_ID(PT_Member_ID)
					.PT_Time(PT_Time).PT_Price(PT_Price).PT_EnrollDate(PT_EnrollDate).
					PT_Date(PT_Date).build();
